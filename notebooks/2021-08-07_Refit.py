...
num_pairs = pair_density * stats.degenerate_pairs(d.spectra, max_dist)

ks_comp = np.array(
    stats.sample_ks_statistics(
        d.spectra,
        d.spectra_fitted,
        k_max=k_max,
        folded=folded,
...
gen = stats.scan_parameters(
    d.spectra, d.spectra_fitted, [500, 1000, 2000], [4, 10, 16], k_max, folded, 15
)
...
def compute_power(spectra, spectra_fitted, max_dists, pair_density, n_reps, k_max):
    ...
    for max_dist in max_dists:
        num_pairs = pair_density * stats.degenerate_pairs(
            len(d.spectra.windows) - 1, max_dist
        )
        ks_comp = np.array(
            stats.sample_ks_statistics(
                d.spectra,
