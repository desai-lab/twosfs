...
ax1.plot(x, lump_sfs(1/np.arange(1,n), maxb)/np.sum(1/np.arange(1,n)), ':k', label='Kingman')
...
for i, alpha in enumerate(A_PLOT):
    ...
    sfs = normalize_sfs(sfs_fitted[(alpha,r)])
    ax1.semilogy(x, lump_sfs(sfs,10), '-x', color=color)#, label="fitted")#, label=model)

    
    plot_model(ax2, model_dict[alpha], label=r"$\alpha={}$".format(alpha), scale=pi_fitted[alpha]/2, color=color)# , color=str(1-alpha))
...
for i, alpha in enumerate(A_PLOT):
    for j, r in enumerate([0, 0.5, 1.0]):
        ...
        ax.pcolormesh(np.arange(1,51),np.arange(1,51),np.log2(f_xibeta/f_fitted), vmin=-2, vmax=2)
        ax.set_yscale("log")
        ax.set_xscale("log")
...
for i, alpha in enumerate(A_PLOT):
    for j, r in enumerate(R_PLOT):
        ...
        ax.pcolormesh(np.arange(1,maxb+3),np.arange(1,maxb+3),np.log2(f_xibeta/f_fitted), vmin=-2, vmax=2, cmap=r"PuOr")
...
#             ax.set_xticks([])
        if j == 0:
            ax.text(xlims[i][0] - 0.95*(xlims[i][1] - xlims[i][0]), 350/2, r"$\alpha={}$".format(alpha))
