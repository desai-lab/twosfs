...
save_path = "/n/home12/efenton/for_windows/newer_2sfs/paper/sequencing_noise/"
beta_color = "darkgreen"


for noise, ax in zip([0.0025, 0.005, 0.01, 0.02], axs.ravel()):

    config = configuration_from_json("../../simulation_parameters.json", root = "../../")
    config.ks_distance_file = insert_folder_name(config.ks_distance_file, f"noise_{noise}", -1)
    ...
    ax.set_title(f"Noise level {noise}", fontsize=7)
...
fig.suptitle(r"Power vs. Tajima's $D$ for different levels of sequencing noise", fontsize=7)
...
fig.savefig(save_path + f"power_noise.pdf")
