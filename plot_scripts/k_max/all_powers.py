...
for k_max, ax in zip([16, 18, 22, 24], axs.ravel()):
    ...
    ax.set_title(r"$k_{max}=$" + str(k_max), fontsize=7)
...
fig.suptitle(r"Power vs. Tajima's $D$ for different values of $k_{max}$", fontsize=7)
...
fig.savefig(save_path + f"power_kmax.pdf")
